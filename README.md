...
面向制造企业数据工程、数据治理、数据分析和平台运维团队的独立产品原型。当前仓库提供一套可本地运行的工程控制台，包括 Node.js API、静态前端工作台、六类岗位登录、接入申请审批、状态流转、报表下载、资产血缘、质量治理和验收中心。

本项目不是通用标题模板页，而是围绕“多源异构大数据处理与质量治理平台”的真实业务结构组织信息架构，覆盖生产数据源接入、采集同步、清洗转换聚合、工作流编排、任务调度、失败重试、数据质量、异常数据、资产目录、血缘分析、权限审计、告警查看和报表下载闭环。

## 当前能力

- 运行控制中心：展示今日处理记录、任务成功率、质量通过率、待处理告警、资产登记量和血缘关系数量。
- 实时数据流：用可视化 DAG 展示生产主题域从工单采集、物料同步、明细清洗、产线聚合到质量检测和资产登记的处理链路。
- 调度监控：查看任务实例、状态筛选、失败重试、暂停调度和人工确认成功。
- 数据接入：查看数据源、采集任务、同步任务，提交接入申请，并由管理员或治理专员审批和完成连通性测试。
- 质量治理：查看质量规则、质量趋势、异常数据问题池，治理专员可关闭已复核问题。
- 资产血缘：查看资产目录、敏感等级、负责人和端到端血缘关系。
- 告警与审计：查看告警事件、文件附件、审计日志和角色权限矩阵。
- 操作中心：生成任务运行日报、质量报告、异常处置清单、血缘影响分析和权限审计报告，并下载当前会话生成的 CSV 报表。
